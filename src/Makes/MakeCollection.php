...
class MakeCollection
{
    ...
    public function make(string $name)
    {
        ...
        $fileName = $metas['Models'] . 'Collection';

        $path = $this->getPath($fileName, 'collection');

        if ($this->files->exists($path)) {
            throw GeneratorException::collectionAlreadyExists($fileName);
        }

        $this->makeDirectory($path);

        $this->files->put($path, $this->compileCollectionStub($metas));
    ...
    protected function bulidColumns($models, &$template){
        ...
        $columns = str_replace("=> '", '=> $item->', $columns);
    ...
    protected function compileCollectionStub(array $metas)
    {
        $stub = $this->files->get(substr(__DIR__, 0, -5) . 'Stubs/Collection.stub');
